...
public class UsageTransactionDAOImpl implements UsageTransactionDAO {
	...
	{
		openConnection();
		ArrayList<UsageTransactionDTO> TrList = new ArrayList<UsageTransactionDTO>();
		try {
			rSet = statement.executeQuery("SELECT * FROM sa40team06.usagetransaction");
			while (rSet.next())
			{
				Timestamp t = rSet.getTimestamp("UsageDate");
				Date d = null;
				if(t != null)
				{
	...
	public boolean insertTransaction(UsageTransactionDTO Tr)
	{
		...
		String sql = "INSERT INTO sa40team06.usagetransaction VALUES ("+
				Tr.getUsageTransId()+
				", "+Tr.getUserId()
				+", "+Tr.getCustomerId()
				+", '"+Tr.getVehicleId()
